...
<main class="w-full max-w-md mx-auto my-auto py-4 relative z-10" x-data="{
    ...
    <div class="text-center mb-6 space-y-2">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Maintenance Portal</h1>
        <p class="text-xs text-slate-400">Sign in to manage service manuals, schematics & aircraft systems</p>
    </div>
    ...
    <div class="rounded-3xl bg-slate-900/90 backdrop-blur-xl border border-slate-800/90 p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <div class="grid grid-cols-2 p-1 bg-slate-950 rounded-2xl border border-slate-800">
            ...
            <a href="{{ route('register') }}" class="py-2.5 text-center text-xs font-semibold text-slate-400 hover:text-white transition-colors">
                Register
            </a>
        </div>

        @if(session('error'))
        <div class="p-3.5 bg-rose-950/60 border border-rose-500/40 rounded-2xl text-xs text-rose-300 flex items-center gap-2.5 shadow-md">
        ...
        @if(session('success'))
        <div class="p-3.5 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-xs text-emerald-300 flex items-center gap-2.5 shadow-md">
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-4">
            ...
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider">Operator Email</label>
                <div class="relative">
            ...
            <!-- Password -->
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider">Access Password</label>
                <div class="relative">
