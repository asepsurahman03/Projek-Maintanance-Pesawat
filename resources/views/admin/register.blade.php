...
<main class="w-full max-w-md mx-auto my-auto py-4 relative z-10" x-data="{
    ...
    <div class="text-center mb-6 space-y-2">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Create Credentials</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400">Join the digital aircraft maintenance and operations team</p>
    </div>
    ...
    <div class="rounded-3xl bg-white dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200 dark:border-slate-800/90 p-6 sm:p-8 shadow-xl dark:shadow-2xl space-y-6 transition-colors duration-200">
        ...
        <div class="grid grid-cols-2 p-1 bg-slate-100 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800">
            <a href="{{ route('login') }}" class="py-2.5 text-center text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                Sign In
            </a>
        ...
        @if($errors->any())
        <div class="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/80 border border-rose-200 dark:border-rose-500/40 text-xs text-rose-700 dark:text-rose-300 space-y-1 shadow-sm">
        ...
        <form method="POST" action="{{ route('register.post') }}" class="space-y-4">
            ...
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Full Name / Engineer</label>
                <div class="relative">
                    <input type="text" name="name" x-model="name" required autofocus
                           placeholder="Captain Alex"
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all">
                </div>
                @error('name')<p class="text-rose-500 dark:text-rose-400 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <!-- Email -->
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Email Address</label>
                <div class="relative">
            ...
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Konfirmasi Password</label>
                <div class="relative">
